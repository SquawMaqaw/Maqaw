...
var users = [{ id: 1, username: 'additt', password: 'MapleAdditt', key: 'cat', email: 'Maple.team@gmail.com'}, {id: 2, username: 'olark', password: 'olark', key: '5678efgh', email: 'OlarkSucks@gmail.com'}];
  ...
  this._app.post('/login',
    ...
    function(req, res, next) {
      ...
      if (self._clients[key] && self._clients[key][id]) {
        ...
        _.each(self._clients[key], function(client) {
          console.log(client.representative);
          if (!client.representative) {
            self._clients[key][id].socket.send(client.id);  
          }  
        });
        res.send(200);
        next();
    ...
    if (!self._clients[key] || !self._clients[key][id]) {
      self._checkKey(key, ip, function(err) {
        if (!err && !self._clients[key][id]) {
          self._clients[key][id] = { token: token, ip: ip, representative: false };
          self._ips[ip]++;
