...
passport.use(new LocalStrategy({
  ...
  function(email, password, done) {
    User.findOne({ email: email }, function(err, user) {
      ...
      if (!user) return done(null, false, { message: 'Incorrect email ' + email });
...
function PeerServer(options) {
  ...
  // Connect to mongodb
  mongoose.connect('mongodb://localhost/' + options.databaseurl);
...
PeerServer.prototype._checkKey = function(key, ip, cb) {
  ...
  User.findOne({ _id: key }, function(err, user) {
    if (!err) {
      if (!_this._clients[key]) {
        _this._clients[key] = {};
      }
      if (!_this._outstanding[key]) {
        _this._outstanding[key] = {};
      }
      if (!_this._ips[ip]) {
        _this._ips[ip] = 0;
      }
      // Check concurrent limit
      if (Object.keys(_this._clients[key]).length >= _this._options.concurrent_limit) {
        cb('Server has reached its concurrent user limit');
        return;
      }
      if (_this._ips[ip] >= _this._options.ip_limit) {
...
PeerServer.prototype._initializeHTTP = function() {
  ...
  // Register
  this._app.post('/register', function(req, res, next) {
    ...
    user.save(function(err) {
      if (err) return res.json(400, { error: 'User registration failed'});

      return res.json(200, { key: user._id, name: user. company });
    })
  ...
  this._app.post('/login', function(req, res, next) {
    passport.authenticate('local', function(err, user, info) {
      ...
      if (!user) {
        res.json(401, { error: 'User login failed'});
      }
      ...
      if (self._clients[key] && self._clients[key][id]) {
        ...
        return res.send(200, { key: user._id, name: user. company });
      }
    })(req, res, next);
  });
