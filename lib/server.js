...
passport.use(new LocalStrategy({
  ...
  function(email, password, done) {
    console.log("going to try to find the user");
...
PeerServer.prototype._initializeHTTP = function() {
  ...
  this._app.post('/register', function(req, res, next) {
    ...
    user.save(function(err) {
      ...
      console.log("returning the user bc successful register");
      return res.json(200, user._id);
    })
  ...
  this._app.post('/login', function(req, res, next) {
    passport.authenticate('local', function(err, user, info) {
      ...
      if (self._clients[key] && self._clients[key][id]) {
        ...
        return res.send(200, JSON.stringify({ type: 'HTTP-ERROR' }));
      }
    })(req, res, next);
  });
